...
// Helper to generate the HTML for the range indicator
...
// Just mock data mappings for existing categories based on image and generic ones
const catData = {
...
// Simple random generator for Dimensions
...
files.forEach(file => {
    ...
    // Process tables
    // We will use regex to find table headers and rows in the 3 sections.
    
    // 1. Categories Table
    // Find <th>Nivel de Riesgo</th> and insert the two new columns before it
    content = content.replace(/(<th[^>]*>% Colaboradores Afectados<\/th>\s*)(<th[^>]*>Nivel de Riesgo<\/th>)/g, 
        '$1<th>Puntaje</th><th>Calificación</th>$2');
        
    // For rows, we need to match the <tr> inside <tbody> and replace.
    // Because regex on HTML is hard, let's do a simple replace with a replacer function.
    content = content.replace(/<tr>\s*<td>(.*?)<\/td>\s*<td class="risk-count">(\d+)<\/td>\s*<td class="percentage">([^<]+)<\/td>\s*<td><span class="risk-badge ([^"]+)">([^<]+)<\/span><\/td>\s*<\/tr>/g, (match, name, count, pct, badgeClass, badgeText) => {
        ...
        // Override badge color to match our mock data for consistency
    ...
    // 2. Domains Table (Análisis por dominio)
    // Domains has: <th>Dominio</th> <th>Categoría</th> <th>Total...</th> <th>%...</th> <th>Nivel...</th>
    // We already replaced headers for all tables with the first regex. Wait, no. The first regex is global so it caught all 3 tables since they all have "% Colaboradores Afectados" then "Nivel de Riesgo". Let's check.
    ...
    // 3. Dimensions Table (Análisis por dimensión general)
    // Dimensions has: 
    // <td class="dimension-cell">...<span>Name</span>...</td>
    // <td class="dimension-domain-info">...</td>
    // <td class="risk-count">...</td>
    ...
    content = content.replace(/<td class="dimension-cell">([\s\S]*?)<\/td>\s*<td class="dimension-domain-info">([^<]+)<\/td>\s*<td class="risk-count">(\d+)<\/td>\s*<td class="percentage">([^<]+)<\/td>\s*<td>\s*<span class="risk-badge ([^"]+)">([^<]+)<\/span>\s*<\/td>/g, (match, cellContent, domInfo, count, pct, badgeClass, badgeText) => {
        let data = getRandomDim(); // Just random for dimensions
        let newBadgeClass = data.color;
        let newBadgeText = newBadgeClass.charAt(0).toUpperCase() + newBadgeClass.slice(1).replace('-', ' ');
